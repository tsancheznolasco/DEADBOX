...
function spawnParticle(x,y,color,speed=2,size=3,life=1,angle=null,spread=Math.PI){
    if(options.reducedEffects&&Math.random()<.55)return;if(particles.length>=ENTITY_LIMITS.particles)return;const p=particlePool.get();if(p)particles.push(p.init(x,y,color,speed,size,life,angle,spread));
}
...
function handleEnemyDeath(z){
    ...
    // La mayor parte del estallido sale en la dirección del disparo y el resto se reparte, para que
    // una muerte se lea como un impacto y no como una desaparición.
    const burst=z.isBoss?46:z.isMiniBoss?32:z.isElite?22:14;
    const away=Number.isFinite(z.lastHitAngle)?z.lastHitAngle+Math.PI:null;
    const burstSpeed=z.isBoss?11:z.isMiniBoss?8:6,burstSize=z.isBoss?7:z.isElite?5:4;
    for(let i=0;i<burst;i++){
        const directed=away!==null&&i<burst*.7;
        spawnParticle(z.x,z.y,z.color,burstSpeed,burstSize,1,directed?away:null,1.5);
    }
    // Sacudida sólo en jefes: en bajas normales sería constante y mareante.
    if(z.isBoss||z.isMiniBoss){shakeTime=z.isBoss?320:170;shakeMagnitude=z.isBoss?15:8;}
...
function recoverGame(){
    ...
    uiRecoveryScreen.classList.add('hidden');uiStartScreen.classList.add('hidden');uiHUD.classList.remove('hidden');startCountdown(()=>{configureRound(currentRound);gameState='PLAYING';lastTime=performance.now();saveProgress('recovered');startMusic();Platform.gameplayStart();});
}
